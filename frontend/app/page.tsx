...
export default function Home() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0D0D0D] text-white">
      {/* Sidebar */}
      <div className={`${isSidebarOpen ? 'w-72' : 'w-0'} bg-[#242424] p-4 flex flex-col border-r border-white/10`}>
        <h1 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500 animate-pulse mb-6">BYTETECK</h1>
      ...
      <div className="flex-1 flex flex-col p-6">
        <div className="flex-1 overflow-y-auto space-y-4">
          ...
          {messages.map((m, i) => (
            <div key={i} className={`p-4 rounded-xl ${m.role === 'user' ? 'bg-[#2A2A2A] ml-auto' : ''}`}>{m.content}</div>
          ))}
          <div ref={messagesEndRef} />
        </div>
